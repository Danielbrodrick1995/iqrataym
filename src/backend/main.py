...
async def recents(session: Session = Depends(get_session)) -> ChatHistoryResponse:
    ...
    if DB_ENABLED:
        ...
        raise HTTPException(
            status_code=400,
            detail="Chat history is not available when DB is disabled. Please try self-hosting the app by following the instructions in the Iqra Taym repository: https://github.com/your-org/iqra-taym",
        )
